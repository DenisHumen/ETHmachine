...
def neura_statistics():
    ...
    if total_in_db > 0:
        ...
        if all_completed:
            ...
            action = ui.menu("Что делать дальше?", [
                ("📎 Экспортировать результаты из БД в CSV", "export"),
                ("🔄 Начать заново — очистить БД и пересобрать", "restart"),
                ("❌ Выход", "cancel"),
            ])
            ...
            if action == 'cancel':
                ...
                if final_csv_path:
                    ...
                    if ui.confirm("Отправить CSV в Telegram?", default=True):
                        try:
            ...
            action = ui.menu("Что делать с незавершёнными задачами?", [
                ("▶️ Продолжить обработку", "continue"),
                ("📎 Экспортировать результаты из БД в CSV", "export"),
                ("🔄 Начать заново — очистить БД", "restart"),
                ("❌ Отмена", "cancel"),
            ])
