...
def main():
    ...
    ui.print_lines(ui.header("ETH Nice Address Generator",
                             "Rust-реализация перебора"))

    num_wallets = ui.ask_int("Сколько nice-кошельков сгенерировать",
                             minimum=1, default=10)
    if num_wallets is None:
        return

    threads = ui.ask_int("Количество потоков (0 — авто)", minimum=0, default=0)
    if threads is None:
        return

    display_process = ui.confirm("Отображать процесс поиска?", default=True)
