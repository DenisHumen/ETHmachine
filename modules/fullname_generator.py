...
def generate_fullnames_menu():
    ...
    ui.print_lines(ui.header("Генератор полных имён", "имя и фамилия"))

    language_choice = ui.choose("На каком языке генерируем?", [
        ("🇷🇺 Русский", "ru"),
        ("🇺🇦 Українська", "ua"),
        ("🇬🇧 English", "eng"),
    ])

    if language_choice in (None, BACK_KEY):
        return

    os_type = platform.system()
    delimiter = get_csv_delimiter()
    ui.print_lines(ui.panel("Параметры", [
        f"имён к генерации: {quantity}"
        f"  (config/config.py → FULLNAME_GENERATOR['QUANTITY'])",
        f"система:          {os_type}",
        f"разделитель CSV:  '{delimiter}'"
        f" {'(точка с запятой)' if delimiter == ';' else '(запятая)'}",
    ]))

    logger.info(f"Генерация {quantity} уникальных имён…")

    try:
        names = generate_unique_fullnames(language_choice, quantity,
                                          gender_mix=True)
        logger.success(f"Сгенерировано {len(names)} уникальных имён")

        preview = [f"{i}. {name}" for i, name in enumerate(names[:10], 1)]
        if len(names) > 10:
            preview.append(f"…и ещё {len(names) - 10}")
        ui.print_lines(ui.panel("Примеры", preview))

        filepath = save_to_csv(names, language_choice)
        logger.success(f"Файл сохранён: {filepath} "
                       f"(формат: fullname{delimiter}language)")
        ui.pause()
