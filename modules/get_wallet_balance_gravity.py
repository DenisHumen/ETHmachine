...
def get_wallet_balance_gravity(wallet_address, rpc_urls):
    try:
        ...
        print("All Gravity Alpha Mainnet RPC URLs failed. Please add more proxies.")
        return None
    except Exception as e:
        print(f"Error: {e}")
        return None

def get_wallet_balance_gravity_with_proxy(wallet_address, rpc_urls, proxies):
    ...
    while rpc_switch_count < max_rpc_switches:
        ...
        try:
            balance = get_wallet_balance_gravity(wallet_address, [rpc_urls[rpc_index]])
