...
# Добавляем локальные импорты или определяем здесь
class Constants:
...
async def create_twitter_client(proxy: str, auth_token: str, ssl_verification: bool):
    ...
    if proxy:
        ...
        #logger.info(f"Используется прокси: {proxy.split('@')[1] if '@' in proxy else proxy}")
    ...
    # Получаем главную страницу для получения ct0
    response = await session.get("https://x.com/home")
    
    # Извлекаем ct0 из cookies
    csrf_token = None
    ...
    # Обновляем заголовки с CSRF токеном
    session.headers["x-csrf-token"] = csrf_token
    
    return session, csrf_token

def retry_async(default_value=None, max_retries=3):
...
class Twitter:
    def __init__(self, account_index: int, auth_token: str, proxy: str, config):
    ...
    async def close(self):
        ...
        if self.session:
            try:
                ...
                logger.debug(f"[{self.account_index}] Session closed successfully")
            except Exception as e:
                None
                #logger.warning(f"[{self.account_index}] Error closing session: {e}")
            finally:
                self.session = None
    ...
    async def initialize(self):
        try:
            self.session, self.csrf_token = await create_twitter_client(
                self.proxy, self.auth_token, self.config.SSL_VERIFICATION
            )
    ...
    async def like(self, tweet_id: str):
        try:
            ...
            if response.status_code != 200:
                logger.error(f"[{self.account_index}] Like failed with status {response.status_code}")
            ...
            try:
                ...
                if "errors" in response_data:
                    ...
                    for error in errors:
                        ...
                        # Проверяем, что лайк уже поставлен
                        if "has already favorited tweet" in error_message:
                            logger.info(f"[{self.account_index}] Лайк уже поставлен на этот твит")
                            return "already_liked"
                        
                        # Проверяем другие ошибки авторизации
                        if "AuthorizationError" in error.get("name", ""):
                            logger.warning(f"[{self.account_index}] Ошибка авторизации при лайке твита")
                            return False
                
                # Проверяем успешность лайка по структуре ответа
                favorite_tweet = response_data.get("data", {}).get("favorite_tweet")
                if favorite_tweet == "Done":
                    logger.success(f"[{self.account_index}] Лайк успешно поставлен на твит: {tweet_id}")
                    return True
                    
            except Exception as json_error:
                logger.warning(f"[{self.account_index}] Не удалось обработать ответ сервера")

            # Дополнительные проверки по тексту ответа
            if '"Done"' in response.text:
                logger.success(f"[{self.account_index}] Лайк успешно поставлен на твит: {tweet_id}")
                return True
            elif (
                "was already liked" in response.text.lower()
                or "has already favorited tweet" in response.text.lower()
                or "already favorited" in response.text.lower()
            ):
                logger.info(f"[{self.account_index}] Лайк уже поставлен на этот твит")
                return "already_liked"
            else:
                logger.warning(f"[{self.account_index}] Неожиданный ответ при попытке поставить лайк")
                return False

        except Exception as e:
            logger.error(f"[{self.account_index}] Ошибка при постановке лайка: {e}")
            return False
    ...
    async def retweet(self, tweet_id: str):
        try:
            ...
            if response.status_code != 200:
                logger.error(f"[{self.account_index}] Retweet failed with status {response.status_code}: {response.text}")
                should_continue = await self._verify_error_response(response.text)
                if not should_continue:
                    return False
            ...
            try:
                ...
                if rest_id:
                    logger.success(f"[{self.account_index}] Successfully retweeted tweet: {tweet_id}")
                    return True
                    
            except Exception as json_error:
                logger.warning(f"[{self.account_index}] Could not parse retweet JSON response: {json_error}")

            if "already retweeted" in response.text.lower() or "You have already retweeted this Tweet" in response.text:
                logger.success(f"[{self.account_index}] Tweet already retweeted: {tweet_id}")
                return True
            else:
                logger.warning(f"[{self.account_index}] Unexpected retweet response: {response.text}")
                return False
    ...
    async def _update_cookies(self):
        try:
            ...
            if ct0 and ct0 != self.csrf_token:
                # Обновляем CSRF токен в заголовках
                self.session.headers["x-csrf-token"] = ct0
                self.csrf_token = ct0
                logger.debug(f"[{self.account_index}] Updated CSRF token to: {ct0}")
