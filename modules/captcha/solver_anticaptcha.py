"""
Anti-Captcha Solver - решение hCaptcha, Turnstile и reCAPTCHA через Anti-Captcha API.
...
class AntiCaptchaSolver:
    ...
    def get_balance(self) -> float:
        ...
        if data.get("errorId", 1) != 0:
            raise Exception(f"Anti-Captcha getBalance error: {data.get('errorDescription', 'Unknown')}")
    ...
    def _solve(self, task: dict, task_type: str, token_field: str = "gRecaptchaResponse") -> Optional[str]:
        ...
        if data.get("errorId", 0) != 0:
            raise Exception(f"Anti-Captcha error: {data.get('errorCode', '')} - {data.get('errorDescription', 'Unknown')}")

        task_id = data.get("taskId")
        if not task_id:
            raise Exception("Anti-Captcha: no taskId returned")
        ...
        for _ in range(120):
            ...
            if result.get("status") == "ready":
                ...
                if token:
                    ...
                    self._solve_count += 1
                    logger.info(f"[Anti-Captcha] {task_type} solved in {elapsed:.1f}s | Solves: {self._solve_count}")
                    return token
                raise Exception(f"No token in Anti-Captcha solution: {solution}")
            if result.get("errorId", 0) != 0:
                raise Exception(f"Anti-Captcha error: {result.get('errorCode', '')} - {result.get('errorDescription', '')}")

        raise Exception("Anti-Captcha timeout waiting for solution")
