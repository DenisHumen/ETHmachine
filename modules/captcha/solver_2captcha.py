...
class TwoCaptchaSolver:
    ...
    def get_balance(self) -> float:
        ...
        if data.get("errorId", 1) != 0:
            raise Exception(f"2Captcha getBalance error: {data.get('errorDescription', 'Unknown')}")
    ...
    def _solve(self, task: dict, task_type: str, token_field: str = "gRecaptchaResponse") -> Optional[str]:
        ...
        if data.get("errorId", 0) != 0:
            raise Exception(f"2Captcha error: {data.get('errorCode', '')} - {data.get('errorDescription', 'Unknown')}")

        task_id = data.get("taskId")
        if not task_id:
            raise Exception("2Captcha: no taskId returned")
        ...
        for _ in range(120):
            ...
            if result.get("status") == "ready":
                ...
                if token:
                    ...
                    self._solve_count += 1
                    logger.info(f"[2Captcha] {task_type} solved in {elapsed:.1f}s | Solves: {self._solve_count}")
                    return token
                raise Exception(f"No token in 2Captcha solution: {solution}")
            if result.get("errorId", 0) != 0:
                raise Exception(f"2Captcha error: {result.get('errorCode', '')} - {result.get('errorDescription', '')}")
