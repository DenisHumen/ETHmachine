...
def _show_stats() -> None:
    ...
    print(f"  {Fore.WHITE}Ghost Faucet — статистика{Style.RESET_ALL}")
...
def _format_summary(stats: dict) -> str:
    parts = []
    for key in ("arrived", "requested", "cooldown", "failed", "in_progress", "pending"):
...
def _run_continue() -> None:
    ...
    log_simple(f"🚰 Ghost Faucet: старт для {total} кошельков "
    ...
    if threads <= 1 or total <= 1:
        ...
        with ThreadPoolExecutor(max_workers=threads, thread_name_prefix="fhenix-faucet") as ex:
