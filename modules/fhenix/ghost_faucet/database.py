"""Ghost Faucet — SQLite БД.

Две таблицы:
  wallet_tasks      — текущий статус задачи (очищается через "очистить и начать заново")
  request_history   — история отправленных заявок на кран (НЕ очищается, нужна для
...
DB_PATH = Path(__file__).resolve().parent.parent.parent.parent / "db" / "fhenix_ghost_faucet.db"
