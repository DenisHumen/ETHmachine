"""Alchemy Base Sepolia faucet worker — основная логика claim per-wallet.

Зеркальная архитектура с modules/fhenix/ghost_faucet/worker.py:
  - per-wallet proxy из data.csv для всех HTTP/RPC запросов
  - SQLite-учёт кулдаунов и истории
  - проверка фактического зачисления баланса по Base Sepolia RPC
"""
...
from config.modules.cfg_fhenix import (
    ...
    ALCHEMY_FAUCET_BALANCE_POLL_INTERVAL,
...
from modules.fhenix.alchemy_faucet.faucet_client import (
    AlchemyFaucetCooldownError,
    AlchemyFaucetError,
    TURNSTILE_PAGEURL,
    TURNSTILE_SITEKEY,
    classify_error,
...
def _get_balance_wei(address: str, proxy: Optional[str] = None) -> Optional[int]:
    """Получить баланс на Base Sepolia через round-robin RPC.
    Все RPC-запросы идут через proxy кошелька."""
    proxy_dict = get_proxy_dict(proxy)
    request_kwargs: dict = {"timeout": 15}
    if proxy_dict:
        request_kwargs["proxies"] = proxy_dict
    last_err: Optional[Exception] = None
    for rpc in ALCHEMY_FAUCET_BASE_SEPOLIA_RPCS:
        try:
...
def _wait_for_arrival(address: str, balance_before: int,
    ...
    deadline = time.time() + ALCHEMY_FAUCET_ARRIVAL_TIMEOUT_MIN * 60
    next_progress = time.time() + 120
    while time.time() < deadline:
        time.sleep(ALCHEMY_FAUCET_BALANCE_POLL_INTERVAL)
...
def _mark_arrived(address: str, balance_before: int, balance_after: int,
    ...
    log_wallet_task(
        address, idx, total,
        f"✅ получено +{delta:.4f} ETH{suffix}",
...
def _attempt_claim(wallet: str, proxy: Optional[str],
    ...
    """Один полный цикл: captcha + POST. Возвращает (ok, tx_hash, message)."""
    token = _solve_captcha(proxy)
    if not token:
        return False, None, "captcha не решена (нет API ключа или сервис вернул ошибку)"

    session = make_session(proxy)
    success, tx_hash, msg = submit_claim(session, wallet, token)
...
def process_wallet(record: dict, idx: int, total: int) -> None:
    ...
    # Локальный кулдаун: возможно, прошлый запрос УЖЕ зачислился — проверим баланс.
    remaining = db.cooldown_remaining(address, ALCHEMY_FAUCET_COOLDOWN_HOURS)
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
        ...
        except AlchemyFaucetCooldownError as e:
            log_wallet_task(address, idx, total,
            ...
            log_wallet_task(address, idx, total,
                            f"⏭ кулдаун от сервера, не зачислилось за {ALCHEMY_FAUCET_ARRIVAL_TIMEOUT_MIN}m",
                            "warning", account_name=account_name)
            return
        except (AlchemyFaucetError, requests.RequestException) as e:
        ...
        if not ok:
            ...
            if kind == "ineligible":
                # Mainnet-balance / activity check провалился — ретрай не спасёт.
                log_wallet_task(address, idx, total,
        ...
        last_error = f"timeout {ALCHEMY_FAUCET_ARRIVAL_TIMEOUT_MIN}m, токены не пришли"
