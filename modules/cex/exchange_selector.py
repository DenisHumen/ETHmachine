...
Позволяет выбрать из настроенных аккаунтов через UI-набор проекта
...
class ExchangeSelector:
    ...
    def select_exchange(self):
        ...
        selected_exchange = ui.menu("Выберите биржу для работы", [
            (f"🏢 {name} ({len(accounts)} аккаунтов)", name)
    ...
    def select_account(self, exchange_name):
        ...
        return ui.menu(f"Выберите аккаунт {exchange_name}", [
